...
public abstract class Controller<T> {
    @Autowired
    private IRedisInfoService redisProdService;
    @Autowired
    ...
    public AccountResponse getAccount(HttpServletRequest request) {
        int account_id = getAccountIdFromTokenRequest(request);
        return redisProdService.getAccount(account_id);
    }

    public UserResp getUser(HttpServletRequest request) {
        int account_id = getAccountIdFromTokenRequest(request);
